...
const translations = {
    en: {
        ...
        verifiedCatalogSource: 'Verified recipe from the Meal Genie collection.',
        showMoreRecipes: 'Show 4 more',
        previousRecipes: 'Previous',
        ...
        downloadAppTitle: 'Get Meal Genie on Android',
        downloadAppHint: 'Install the app on your phone — same logo, full experience.',
        downloadAppBtn: 'Download Meal Genie',
    ...
    ur: {
        ...
        verifiedCatalogSource: 'Meal Genie مجموعہ سے تصدیق شدہ ترکیب۔',
        showMoreRecipes: '۴ مزید دکھائیں',
        previousRecipes: 'پچھلا',
        ...
        downloadAppTitle: 'Android پر Meal Genie حاصل کریں',
        downloadAppHint: 'ایپ اپنے فون پر انسٹال کریں — وہی لوگو، مکمل تجربہ۔',
        downloadAppBtn: 'Meal Genie ڈاؤن لوڈ کریں',
...
function applyLanguage() {
    ...
    const downloadTitle = document.getElementById('settingsDownloadTitle');
    if (downloadTitle) downloadTitle.textContent = tr('downloadAppTitle');
    const downloadHint = document.getElementById('downloadAppHint');
    if (downloadHint) downloadHint.textContent = tr('downloadAppHint');
    const downloadBtn = document.getElementById('downloadAppBtn');
    if (downloadBtn) downloadBtn.textContent = tr('downloadAppBtn');
...
function refreshSpotlightRecipes() {
    ...
    spotlightRecipeIds = shuffled.slice(0, 12).map(r => r.id);
}
...
function updateResults() {
    ...
    if (selectedIngredients.length === 0) {
        ...
        document.getElementById('resultsContainer').innerHTML = renderSection(tr('tryTheseRecipes'), spotlight, { markAllVerified: true });
        return;
    }
    ...
    if (hasMatch) {
        html += renderSection(tr('verifiedRecipes'), matching, { markAllVerified: true });
    }
...
function paginateResultsSection(sectionKey, page) {
    resultSectionPages[sectionKey] = Math.max(0, page);
    updateResults();
}

function isVerifiedRecipe(recipe) {
    if (!recipe || recipe.isGenerated) return false;
    return Boolean(recipe.isSpoonacular || recipes.some(r => r.id === recipe.id));
}

function renderSection(title, items, options = {}) {
    const opts = typeof options === 'boolean' ? { isCustom: options } : options;
    const { isCustom = false, markAllVerified = false } = opts;
    const sectionKey = getSectionKey(title);
    const page = resultSectionPages[sectionKey] || 0;
    const totalPages = Math.max(1, Math.ceil(items.length / RECIPES_PER_PAGE));
    const safePage = Math.min(page, totalPages - 1);
    if (safePage !== page) resultSectionPages[sectionKey] = safePage;
    const start = safePage * RECIPES_PER_PAGE;
    const visible = items.slice(start, start + RECIPES_PER_PAGE);
    ...
    return `
        <div class="result-section">
            ...
            <div class="result-grid">
                ${visible.map(r => {
                    const showVerified = markAllVerified || r.isSpoonacular || (!r.isGenerated && recipes.some(x => x.id === r.id));
                    return `
                    <div class="result-card ${r.isGenerated ? 'generated' : ''} ${showVerified ? 'verified' : ''}" onclick="openRecipeFromResults(${r.id})">
                        ...
                        <div class="result-cuisine">${escapeHtml(r.cuisine)}</div>
                        ${isCustom ? `<span class="custom-badge">${tr('customBadge')}</span>` : ''}
                        ${showVerified ? `<span class="verified-badge">${tr('verifiedBadge')}</span>` : ''}
                    </div>
                `;
                }).join('')}
            </div>
            ${items.length > RECIPES_PER_PAGE ? `
                <div class="result-pagination">
                    <button type="button" class="result-page-btn" onclick="paginateResultsSection('${sectionKey}', ${safePage - 1})" ${safePage === 0 ? 'disabled' : ''}>← ${tr('previousRecipes')}</button>
                    <span class="result-page-indicator">${tr('pageIndicator', { current: safePage + 1, total: totalPages })}</span>
                    <button type="button" class="result-page-btn" onclick="paginateResultsSection('${sectionKey}', ${safePage + 1})" ${safePage >= totalPages - 1 ? 'disabled' : ''}>${tr('showMoreRecipes')} →</button>
                </div>
            ` : ''}
        </div>
    `;
...
async function showRecipeAsync(id) {
    ...
    const verifiedBadge = isVerifiedRecipe(recipe) ?
    ...
    } else if (recipes.some(r => r.id === recipe.id)) {
        sourceEl.hidden = false;
        sourceEl.innerHTML = `<span class="source-note">${tr('verifiedCatalogSource')}</span>`;
    } else {
...
function toggleFridge() {
    ...
    if (panel.classList.contains('open')) {
        ...
        ['moreFeaturesPanel', 'pantryPanel', 'plannerPanel', 'groceryPanel'].forEach((id) => {
...
async function detectFridgeIngredients(imageData) {
    // Prefer dedicated fridge-scan API (handles blurry images + confidence).
    try {
        ...
        if (response.ok) {
            ...
            const detected = Array.isArray(payload.ingredients) ? payload.ingredients : [];
            const ids = detected
                .map((row) => {
                    const name = String(row.name || row || '').toLowerCase();
                    const byId = getIngredientById(name) || getIngredientById(name.replace(/\s+/g, '_')) || getIngredientById(name.replace(/\s+/g, '-'));
                    if (byId) return byId.id;
                    return getIngredientByName(name)?.id;
                })
                .filter(Boolean);
            if (ids.length) return [...new Set(ids)].slice(0, 12);
            if (payload.blurry || payload.error) {
                // Fall through to manual-friendly simulate only if empty.
            }
        }
    } catch (_) {
        /* fall through */
...
function applyIngredientIds(ids) {
    const next = Array.isArray(ids) ? ids.filter(Boolean) : [];
    selectedIngredients = [...new Set(next)];
    leftoverMode = true;
    renderIngredients();
    updateResults();
    updateGenerateRecipeFab();
...
export function bindMealGenieGlobals() {
    ...
    Object.assign(window, {
        ...
        setFridgePref,
        generateFridgeMenu,
        applyFridgeToKitchen,
        toggleFridgeIng,
        openFridgeRecipe,
        applyIngredientIds,
    });
    window.__mealGenieIngredients = ingredients;
}
...
export function initMealGenieApp() {
    ...
    import('@/lib/mealgenie-extra-features')
        .then((mod) => mod.bindExtraFeatureGlobals())
        .catch(() => {});
    hydrateStorageState();
